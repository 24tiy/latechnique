...
function Mushroom({ idx }: { idx: number }) {
  ...
  const s = useRef({
    ...
    px: (Math.random()-0.5) * CFG.fieldWidth * 0.5,
    pz: -2 - Math.random() * 3,
  });
  ...
  useFrame((_, dt) => {
    ...
    if (o.phase === 'dormant') {
      ...
      if (o.timer >= CFG.mushroomSpawnTime) {
        ...
        o.px = (Math.random()-0.5)*CFG.fieldWidth*0.5;
        o.pz = -2-Math.random()*3;
        o.max = 0.25+Math.random()*0.35;
        g.position.set(o.px, 0, o.pz);
        g.visible = true;
      }
    } else if (o.phase === 'grow') {
      const t = Math.min(o.timer / CFG.mushroomGrowTime, 1);
      g.scale.setScalar((1-(1-t)*(1-t)*(1-t)) * o.max);
      g.position.y = -0.02 + t * 0.02;
...
function Butterfly({ idx }: { idx: number }) {
  ...
  const st = useRef({ t: Math.random(), sp: 0.25+Math.random()*0.25, ph: Math.random()*6.28, path: mkPath() });
  ...
  useFrame(({ clock }, dt) => {
    ...
    if (o.t >= 1) { o.t = 0; o.path = mkPath(); }
    const p = crPt(o.path, o.t);
...
function mkPath(): THREE.Vector3[] {
  const p: THREE.Vector3[] = [];
  for (let i = 0; i < 6; i++) p.push(new THREE.Vector3((Math.random()-0.5)*CFG.fieldWidth*0.4, 0.4+Math.random()*1.0, -1.5-Math.random()*3));
  return p;
}

function crPt(pts: THREE.Vector3[], t: number): THREE.Vector3 {
...
function Pollen() {
  ...
  const { pos, vel } = useMemo(() => {
    ...
    for (let i = 0; i < CFG.particleCount; i++) {
      p[i*3]=(Math.random()-.5)*14; p[i*3+1]=0.2+Math.random()*1.5; p[i*3+2]=-1-Math.random()*4;
      v[i*3]=(Math.random()-.5)*.04; v[i*3+1]=(Math.random()-.3)*.012; v[i*3+2]=(Math.random()-.5)*.02;
    }
    return { pos: p, vel: v };
  }, []);
  ...
  return (
    <points ref={ref}>
      <bufferGeometry><bufferAttribute attach="attributes-position" array={pos} count={CFG.particleCount} itemSize={3}/></bufferGeometry>
...
function Ground() {
  return (
    <mesh rotation={[-Math.PI/2,0,0]} position={[0,-0.02,-3.5]}>
      <planeGeometry args={[CFG.fieldWidth+10, CFG.fieldDepth+10]}/>
...
/* ── CAMERA ──
   Positioned at ground level, looking straight ahead.
   Grass base (y=0) is at the BOTTOM of the viewport.
   Grass tips (y=1) rise UP into frame.
   Camera y = 0.15 (just above ground), looking at y=0.5 ahead.
*/
function Cam() {
...
export default function MeadowScene() {
  return (
    <Canvas
      camera={{ position: [0, 0.15, 1], fov: 55, near: 0.1, far: 25 }}
